...
map<int,int> map1[110],map2[110],max_map,min_map_num;
    ...
    for(t=0;t<90;t++)
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            for(j=0;p[j]*p[j]<=temp;j++)
            {
                if(temp%p[j]==0)
                {
                    ...
                    min_map_num[p[j]]=INT_MAX;
                }
            }
            if(temp>1)
            {
                map2[i][temp]++;
                min_map_num[temp]=INT_MAX;
            }
        }
       /* cout<<"map2"<<endl;
        for(i=0;i<n;i++){
       ...
       /* cout<<"map2"<<endl;
        ...
        for(it=max_map.begin();it!=max_map.end();it++)
        {
            ...
            {
                int res=min(min_map_num[it->first],it->second);
        ...
        InfInt mul=__gcd(ans_num,ans_den);
        ans_num/=mul;
        ans_den/=mul;
