...
}p,q,pp,qq,ans_num,ans_den,tn,td,cmn,x,y;
...
int main()
{
    ...
    for(t=0;t<test;t++)
    {
        ...
        for(i=0;i<n;i++){
            ...
            scanf("%d%d",&frac[i].first,&frac[i].second);
            same=__gcd(frac[i].first,frac[i].second);
            frac[i].first/=same;
            frac[i].second/=same;
        }


           // scanf("%d%d",&num[i],&frac[i].second);

        ans_num=frac[0].first; ans_den=frac[0].second;
