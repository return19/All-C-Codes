...
int main()
{
    ...
    for(t=0;t<70;t++)
    {
        ...
        {
            cin>>num[i]>>den[i];
            same=__gcd(num[i],den[i]);
            num[i]/=same;
            den[i]/=same;
            vis[i]=0;
        }

        ans_num=num[0]; ans_den=den[0];
        vis[0]=1;
        ...
        for(i=0;i<n;i++)
        {
            ...
            ans_num= ans_num*(dval) + nval*ans_den;
            ans_den=ans_den * dval;

            for(j=0;j<p.size();j++)
            {
                while(ans_num%p[j]==0 && ans_den%p[j]==0)
                {
                    ans_num/=p[j];
                    ans_den/=p[j];
                }
