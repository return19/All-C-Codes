...
map<long long,long long> map1[110],map2[110],max_map,min_map_num;
...
int main()
{
    ...
    for(t=0;t<test;t++)
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            for(j=0;p[j]*p[j]<=temp;j++)
                if(temp%p[j]==0)
                {
                    ...
                    map1[i][p[j]]=c;
                    max_map[j]=max(max_map[p[j]],c);
                }
            if(temp>1)
            {
                map1[i][temp]=1;
                max_map[temp]=max(max_map[temp],1);
            }
        }
        ...
        for(i=0;i<n;i++)
        {
            ...
            {
                map2[i][it->first] -= it->second;

                if(map2[i][it->first]==0)
                    map2[i].erase(it->first);
            }
        }
        ...
        for(i=0;i<n;i++)
        {
            ...
            for(j=0;p[j]*p[j]<=temp;j++)
            {
                if(temp%p[j]==0)
                {
                    ...
                    min_map_num[p[j]]=LLONG_MAX;
                }
            }
            if(temp>1)
            {
                map2[i][temp]++;
                min_map_num[temp]=LLONG_MAX;
            }
        }


        for(it=min_map_num.begin();it!=min_map_num.end();it++)
        {
            for(i=0;i<n;i++)
                    min_map_num[it->first]=min(min_map_num[it->first],map2[i][it->first]);
           // if(min_map_num[it->first]==0)
             //   min_map_num.erase(it->first);
        }

        for(i=0;i<n;i++)
        {
            for(it=map2[i].begin();it!=map2[i].end();it++)
            {
                map2[i][it->first] -= min_map_num[it->first];
            }
        }
        ...
        for(i=0;i<n;i++)
        {
            ...
            for(it=map2[i].begin();it!=map2[i].end();it++)
            {
                temp *= power(it->first,it->second);
            }
            res_num += temp;
        }

        /*long long temp=res_num;
        for(i=2;i*i<=temp;i++)
        {
            if(temp%i==0)
            {
        ...
        for(it=max_map.begin();it!=max_map.end();it++)
        {
            if(min_map_num.find(it->first)!=min_map_num.end())
            {
                long long res=min(min_map_num[it->first],it->second);
                min_map_num[it->first] -= res;
                max_map[it->first] -= res;
            }
        }*/
        ...
        for(it=max_map.begin();it!=max_map.end();it++)
        {
            ans_den *= power(it->first,it->second);
        }

        ans_num=1;
        for(it=min_map_num.begin();it!=min_map_num.end();it++)
        {
            ans_num *= power(it->first,it->second);
        }

        ans_num*=res_num;


        /*to do : factorize res_num and then again subtract from den*/

       /* long long mul=__gcd(ans_num,ans_den);
        ans_num/=mul;
        ans_den/=mul;*/
