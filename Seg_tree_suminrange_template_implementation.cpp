...
public:
  void merge(struct segtree_node& a,struct segtree_node& b)
  {
      sum=a.sum+b.sum;
  }
  /*void split(struct segtree_node& a,struct segtree_node& b)
  {
...
segtree_node arr[100010];
segtree_node identity;
segtree_node ans,changed;

void update_single_subtree(segtree_node& n)
{
    n.sum=changed.sum;
}
    ...
    for(i=0;i<test;i++)
    {
        ...
        for(j=0;j<q;j++)
        {
            ...
            else if(t==2)
            {
                a--;
                changed.sum=b;
                s.update<update_single_subtree>(a);
            }
            else if(t==3)
            {
                scanf("%d",&c);
                ...
                b--;
                changed.sum=c;
                s.update<update_single_subtree>(a,b);
            }
